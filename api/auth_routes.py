...
# ===== API Key 子路由（2026-09-15，cloudsol CLI 公开分发） =====

@router.post("/api-key", dependencies=[Depends(rate_limit(5, 300))])
async def create_api_key(body: dict, current_user: dict = Depends(get_current_user)):
    """签发 API Key：明文仅本次响应返回一次；网页 JWT 身份才可签发（key 不能签发 key）。"""
    if current_user.get("_auth_via") == "api_key":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="API Key 无权签发新 Key，请使用网页账号登录后操作",
        )
    from app.services.api_key_service import ApiKeyService
    name = str((body or {}).get("name") or "")[:50]
    result = ApiKeyService.issue_key(current_user["id"], name)
    if not result.get("success"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=result.get("message", "签发失败"))
    return {
        "id": result["id"], "key": result["key"], "key_prefix": result["key_prefix"],
        "daily_limit": result["daily_limit"],
        "note": "明文 Key 仅此一次返回，请立即保存到 CLI（cloudsol login --key <key>）",
    }

@router.get("/api-keys")
async def list_api_keys(current_user: dict = Depends(get_current_user)):
    from app.services.api_key_service import ApiKeyService
    return {"keys": ApiKeyService.list_keys(current_user["id"])}

@router.delete("/api-key/{key_id}")
async def revoke_api_key(key_id: int, current_user: dict = Depends(get_current_user)):
    from app.services.api_key_service import ApiKeyService
    ok = ApiKeyService.revoke_key(current_user["id"], key_id)
    if not ok:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="密钥不存在、无权操作或已吊销")
    return {"ok": True, "message": "已吊销"}
