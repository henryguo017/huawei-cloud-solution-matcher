...
css_root = (Path(frontend_path) / "css").resolve()
...
@app.get("/css/{filename:path}", tags=["前端"])
async def serve_css(filename: str):
    """
    返回 css/ 目录下的样式文件（Agent 视图独立样式由此路由提供）
    """
    css_path = (css_root / filename).resolve()
    if not css_path.is_relative_to(css_root) or not css_path.is_file():
        raise HTTPException(status_code=404)
    if os.path.exists(css_path):
        return FileResponse(css_path, media_type="text/css")
    raise HTTPException(status_code=404)
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        reload=True,
        reload_excludes=[
            "_*",               # 根目录验证/调试脚本，避免写脚本触发重载
            "*.log",            # 日志
            "*.png", "*.jpg",   # 截图
            "*.md", "*.txt",    # 文档/输出
            ".workbuddy",       # 记忆目录
            "frontend/_dev",    # 开发辅助页
            "data",             # 数据目录（DB/知识库，无代码）
        ],
        log_level="info"
    )
