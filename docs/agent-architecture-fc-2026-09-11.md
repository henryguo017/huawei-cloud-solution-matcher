...
| 指标 | 定义 | 目标 | 证明什么 |
|---|---|---|---|
| A1 自主拆解率 | **走 FC 引擎的运行**中产出可执行计划（plan 步数 ≥2）的比例 | ≥70% | C1/C4 |
...
> **A1 口径裁定（2026-09-13，依据 S4 30 例对照）**：A1 的**分母只数「真正走 FC 引擎」的运行**，不把按 D5 退管 legacy 的 `data`/`knowledge`/部分 `mixed` 用例算进去。原因：`fc` 臂的一次实测里本就合法地混着两套引擎的行，若用「全样本」当分母，等于拿一个恒不可能满分的指标卡门禁（S4 实测全样本 0.621 ❌ 而 FC 引擎口径 0.727 ✅）。全样本数值仍保留在报告里供参考，但**判定以 FC 引擎口径为准**。
