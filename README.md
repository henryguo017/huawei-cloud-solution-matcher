...
- **华为红 12 页 PPT 导出引擎**：方案一键生成满版商务模板 PPT（封面/目录/架构/量化收益/成本/竞品对比等 12 页），成本数字自动写入，双通道入口（结果页按钮 + Agent 口语"给我生成一个PPT"），生成门禁校验+自动重试。
- **Agent 对话工作台**：Plan 实时点亮、工具执行可见、成本计算器真实算价、制造/医疗/政务/金融/零售五行业专家口径、出稿前质量自检、跨会话记忆。
- **钉钉 / 飞书群通知**：方案完成自动推送免登录分享卡片（30 天有效），按账号隔离可绑多群。
- **客户管理 CRM**：客户档案 + 方案时间线一键归集，匹配自动参考客户行业精准检索。
- **快速体验**：顶栏「体验」一键跑三个行业 Demo，匿名可用。

### v2.4.0（2026-08）

- **顶栏「最新更新」滚动横幅**：顶部持续展示平台最新动态（常驻显示，悬停暂停滚动）。
- **顶栏天气条**：默认按浏览器定位显示当地天气，可搜索切换国内任意城市（和风天气数据源，key 存服务端不泄露）；移动端自动隐藏。
- **资讯弹窗三标签页**：顶栏「资讯」按钮 → ①行业资讯（9 个 RSS 源，相关性白/黑名单过滤）②华为云动态（华为云社区博客自动聚合）③行业展会（AIWW + IDCTalk 自动聚合 + 人工精选兜底，已结束自动清理，均 24h 自动更新）。
- **竞品分析多选**：可同时选择 2 家竞品，与华为云生成三方横向对比表（产品 / 价格 / 生态 / 安全 / 案例 / 服务六维），投标横评常用。
- **方案摘要卡片**：匹配结果顶部渲染「方案摘要」卡片（价值主张 + 核心要点），导出 Word / PDF 与分享链接自动附带；正文执行摘要改为结构化输出（价值主张一行 + 要点列表）。
- 列表样式统一：方案正文 / 分享页 / 导出文档的列表项均无圆点、无编号，仅保留缩进层级。
...
> 📷 **更多界面**：顶栏「资讯」弹窗（行业资讯 / 华为云动态 / 行业展会三标签页）、竞品多选三方横评表、天气条等新功能截图，可在登录 `https://cloudsol.cn` 后自行截图补充至 `test_shots/` 目录。
...
- **顶栏天气条**：浏览器定位 + 城市搜索，出差见客户前顺手看天气（和风数据源，key 存服务端）。
- **资讯三标签页**：行业资讯（9 RSS 源）/ 华为云官方动态 / 行业展会日历，全部 24h 自动更新，售前随时掌握行业与竞品动态。
- **竞品多选横评**：12 家竞品可多选 2 家，与华为云一键生成三方横向对比表，投标横评常用。
- **方案摘要卡片**：匹配结果顶部一页纸摘要（价值主张 + 核心要点），导出与分享自动附带。
...
- **多选横评**：可同时选择 2 家竞品，与华为云生成三方横向对比表（产品能力 / 价格策略 / 生态建设 / 安全合规 / 行业案例 / 服务支持六维），投标横评场景常用。
...
### 12. 顶栏日常工具（天气 / 资讯 / 更新横幅）
- **天气条**：顶栏左侧，默认 `navigator.geolocation` 定位 → 后端反查城市 + 天气（和风天气，key 存服务端 `.env` 不泄露）；点击展开面板可搜索切换国内任意城市，选中城市 localStorage 记忆；定位失败 / API 不可用时静默降级提示，不阻塞页面。
- **资讯弹窗**（顶栏「资讯」按钮，三个标签页）：
  1. **行业资讯**：聚合 9 个科技媒体 RSS（IT之家 / 36氪 / 量子位 / InfoQ / 开源中国 / 爱范儿 / 极客公园 / 虎嗅 / 少数派），跨源标题去重 + 相关性白/黑名单过滤（只保留与云计算 / AI / 行业方案沾边的新闻）；
  2. **华为云动态**：聚合华为云社区博客（bbs.huaweicloud.com，自家域名服务端渲染，无 WAF），覆盖产品发布 / 版本更新 / 优惠活动 / 认证考试；
  3. **行业展会**：自动聚合 AIWW / IDCTalk 展会聚合站（只保留未来活动）+ 人工精选年度大会兜底（华为全联接 / 云栖 / 腾讯数字生态等，链接指向官网），**已结束的展会自动清理**。
  - 三者均为 **24h 进程内 TTL 缓存**（首次访问触发抓取，之后 24h 内读缓存秒开），单源失败自动跳过，全失败降级返回旧缓存或空态提示。
- **更新横幅**：顶栏正下方滚动条，展示平台最近更新动态（常驻显示、悬停暂停、无关闭按钮）。
...
| 天气 | 和风天气（服务端代理，key 存 `.env`） |
| 资讯 / 展会 | RSS 聚合 + 华为云社区博客（bbs.huaweicloud.com）+ AIWW/IDCTalk 展会聚合站，24h TTL 缓存 |
...
| 天气 | `GET /api/weather`（city 或 lat+lng）、`GET /api/weather/search`（城市搜索） |
| 资讯 | `GET /api/news`（行业 RSS 聚合，24h 缓存）、`GET /api/huawei-news`（华为云社区动态）、`GET /api/events`（行业展会，自动聚合 + 人工兜底） |
...
- [x] **资讯三标签页 + 展会自动聚合**（2026-08）：行业资讯 / 华为云动态 / 行业展会，24h 自动更新。
- [x] **顶栏日常工具**（2026-08）：天气条（和风）、更新横幅。
- [x] **竞品多选横评 + 方案摘要卡片**（2026-08）。
...
- [ ] **代码拆分**：`api/routes.py`（~4000 行）与 `frontend/script.js`（~9000 行）已达维护临界点，后续新功能建议独立路由文件（已有 auth/export 拆分先例）。
