...
**新增：**
- 📧 密码找回功能（忘记密码 → 邮箱接收重置链接 → 设置新密码）
- 📧 163 邮箱 SMTP 集成（免费，每天约 50 封额度，零成本）
- 📧 新用户注册强制填写邮箱（Pydantic `EmailStr` 必填校验）
- 💡 老用户（无邮箱）登录后弹窗引导绑定邮箱（7 天内不再提示）

**优化：**
- 重置链接有效期 30 分钟（安全设计）
- 重置链接用一次即失效（防重复使用）
- 同一邮箱 1 分钟内只能发一次（防邮件轰炸）
- 邮件发送使用 Python 标准库 `smtplib`，无额外第三方依赖

**修复：**
- 新增 SPA 回退路由，深链接 `/reset-password?token=xxx` 不再返回 404
- 邮箱链接点击后通过 `index.html` 内联脚本自动弹出重置密码窗口（不受静态资源缓存影响）
- 带 token 访问时强制隐藏欢迎页，避免遮挡重置弹窗
- 重置密码完成 / 登录完成后不再重复弹出欢迎页（欢迎页改为每天仅显示一次，基于 localStorage 当日日期）
