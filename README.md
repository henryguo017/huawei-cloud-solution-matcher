...
<p align="center"><i>产品图谱：可视化浏览华为云产品体系（50 产品节点 / 9 分类 / 含产品价目卡片），作为方案设计的参考视图</i></p>
...
- **成本参考卡片**：产品图谱内置公开价目卡片，支持月/年切换、地区统一、自定义产品行，报告可附成本对比表。
- **成就系统 + 数据看板**：把"用工具"变成有反馈的事，一眼看清知识库家底。
- **内置 AI 助手**：四路智能分流回答「平台怎么用 / 我的个人知识 / 云与竞品业务 / 闲聊」四类问题，单次对话长上下文 + 一键新对话清空。
...
### 10. 成就系统 / 数据看板 / 产品图谱 / 成本参考
- 基于使用行为解锁成就徽章；看板展示文档数、行业覆盖与分布。
- 产品图谱可视化浏览华为云产品体系（50 产品节点 / 9 分类），节点含能力、场景、优势与关联产品。
- **成本参考卡片**：内置公开价目骨架，支持月费 / 年费（85 折）切换、地区统一「华北-北京四」、自定义添加产品行；导出报告可附成本对比表。

### 11. 内置 AI 问答助手（四路智能分流）
- 顶部面板，按问题类型自动分流到四条路由：
  1. **平台使用向导**：基于内置平台知识回答「方案匹配怎么用 / 怎么导出报告」等操作问题。
  2. **个人知识（登录后）**：只基于你的客户档案（`clients` 表）+ 你真实上传的私有文档作答，未录入时如实说明、不拿公开案例充数、不泄漏竞品。
  3. **云计算 / 技术业务 RAG**：检索知识库回答华为云与竞品的产品、方案、对比类问题。
  4. **通用闲聊**：其余话题走纯大模型自然对话。
- **单次对话长上下文**：按 token 预算（默认 30000）动态截断历史，单会话内跨多轮保持连贯。
- **一键「新对话」**：清空上下文，开始全新会话。
- 全部回复统一后处理清洗 Markdown 符号，纯文本呈现；身份锚定为「cloudsol.cn 内置 AI 助手」，不冒充任何第三方大模型。
...
| 历史 / 收藏 | 历史方案版本化、收藏管理 |
| 成就 / 看板 | 成就解锁查询、知识库统计 |
| AI 助手 | `POST /api/ai/chat`（四路智能分流问答） |
| 成本参考 | `GET /api/pricing/reference`（行业价目骨架）、`GET /api/pricing/products`（产品价目全量） |
