...
# ============================================================
# 5b. 竞品对比格式（轻量，5 章）—— 供竞品对比意图复用，避免套 14 章方案书
# ============================================================
def build_compare_block() -> str:
    return (
        "请严格按以下 Markdown 格式回答（章节标题必须带 ## 编号；"
        "禁止输出图片/mermaid/HTML/外部链接图片；用纯文字分层描述）：\n"
        "## 1. 对比结论（一句话）\n"
        "（先给结论：在用户关注的维度上，华为云与[竞品]谁更占优，尽量量化）\n"
        "## 2. 核心维度对比\n"
        "（用 Markdown 表格：| 维度 | 华为云 | [竞品] | 结论 |；维度含：产品矩阵 / 价格与商务 / "
        "生态与集成 / 合规(政务国资) / 行业方案 / 服务支持；每项标注[资料N]或『需进一步核实』）\n"
        "## 3. 华为云差异化优势\n"
        "（3-5 条，对应竞品短板，逐条标注[资料N]）\n"
        "## 4. 适用场景建议\n"
        "（什么客户/场景优先华为云，什么场景竞品更合适，客观中立）\n"
        "## 5. 选型与下一步\n"
        "（给客户的选型建议 + 后续动作：技术交流 / POC）\n"
        "【输出前自检】第 2/3 章每条含数字/案例的句子末尾须有 [资料N] 或『需进一步核实』；"
        "不得编造竞品弱点，对比须客观。\n"
    )


# ============================================================
# 5c. Agent 自主结构格式（P0 模板降权，2026-08-26）
#     不强制 14 章骨架，只给"应覆盖的元素清单"作为参考，让 LLM 自主组织输出结构。
#     用于 Agent 模式的 solution / competitor 意图，消除"模板填充器"包装感。
# ============================================================
def build_agent_format_block() -> str:
    return (
        "【输出要求】\n"
        "1. 用 Markdown 组织回答：章节标题用 ## 编号；禁止输出图片/mermaid/HTML/外部链接图片；"
        "表格/列表可按内容需要自由使用。\n"
        "2. 结构由你自主决定——只要覆盖以下『售前方案应包含的元素』（按重要性取舍，不必全列）：\n"
        "   - 客户需求与痛点分析（可量化）\n"
        "   - 方案总体架构或解决思路（分层/分模块描述）\n"
        "   - 推荐的产品组合与各自角色（可用表格）\n"
        "   - 核心能力与客户价值（降本/增效/合规/控风险）\n"
        "   - 若检索到竞品资料：客观对比；否则写明『未指定竞品』\n"
        "   - 实施路径或落地步骤（分阶段）\n"
        "   - 风险与应对、下一步行动建议\n"
        "3. 语气是『售前顾问向客户讲解』，不是填空模板；开头直接用结论或价值主张切入，"
        "避免『以下是……方案』这类空话。\n"
        "4. 【防幻觉】引用检索资料时在句末标注 [资料N]；含数字/案例的句子必须有 [资料N] 或『需进一步核实』；"
        "不编造客户名/项目名/精确数值。\n"
    )
