...
_FACT_ITEM_RE = re.compile(
    ...
    # 单位单组交替且多字单位优先（2026-09-15 修复：「300万元」此前被「万」先行
    # 吞掉量级截成「300万」；合并为单交替并按长度排序后完整吃下「万元/亿元」）
    r'(?P<num>\d+(?:\.\d+)?(?:\s*(?:万元|亿元|万|亿|台|套|座|家|人|名|条|个|厂|%|pct|㎡|个月|月|周|年|天))?'
    r'(?:以上|以内|左右|出头)?)'
)
# 标签尾部虚词/连接词剥除（避免「OEE仅」「工厂和」这类脏标签）
_LABEL_TAIL_TRIM = '的约仅为超达到有近在把将希望先需还共计划之中上和与及或'
