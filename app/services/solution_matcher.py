...
class SolutionMatcherService:
    ...
    async def _analyze_demand(self, customer_demand: str) -> Dict[str, Any]:
        """复用共享需求结构化服务 analyze_demand，把模糊需求结构化。失败则静默返回空。"""
        try:
            from app.services.demand_analyzer import analyze_demand_structured
            return await analyze_demand_structured(customer_demand)
        except Exception as e:
    ...
    def _build_prompt(self, question: str, context: str, industry: str,
                       ...
                       client_context: str = "", format_mode: str = "solution") -> str:
        ...
        # B 方案：竞品对比意图用轻量对比格式，其余仍用 14 章售前方案书。
        # P0 模板降权（2026-08-26）：format_mode="agent" → 自主结构 + 元素建议（不强制 14 章），
        # 让 Agent 输出更像"自主智能体的交付"，而不是"模板填充器"。
        if format_mode == "competitor":
            format_block = build_compare_block()
        elif format_mode == "agent":
            from app.services.solution_prompt import build_agent_format_block
            format_block = build_agent_format_block()
        else:
    ...
    async def generate_enhanced(
        ...
        Args:
            ...
            format_mode: "solution"（14章售前方案书）| "competitor"（轻量对比格式）
    ...
    async def generate_enhanced_stream(
        ...
        """流式版 generate_enhanced（Agent 模式专用）：逐 token 回调 on_delta，返回与 generate_enhanced 同形状 dict。

        经典模式走 match()/match_stream()，不调用本方法，故本方法为纯新增、对经典零影响。
        """
        playbook_text = self._playbook_text(industry or "")
        final_prompt = self._build_prompt(
            question=demand,
        ...
        collected: list = []
        # 与 generate_enhanced 一致：不传 model（保持 pro 模型）
        async for delta in get_llm_response_stream(final_prompt):
            if delta:
                collected.append(delta)
                if on_delta:
                    await on_delta(delta)
        answer = "".join(collected)
        # 追加参考资料节（与 generate_enhanced 一致，避免悬空 [资料N]）
        if "参考资料" not in answer:
            refs = build_references_section(context)
            if refs:
                answer = answer.rstrip() + "\n" + refs
