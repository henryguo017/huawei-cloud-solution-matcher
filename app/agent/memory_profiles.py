...
def build_memory_context(user_id: int, query: str,
    ...
    # 业务闭环 E2E 修复（2026-09-13）：跨会话记忆污染——真实场景实测，用户本轮明确
    # 在谈汽车零部件，模型却把历史会话的园区场景当成当前需求出了整套交付物。
    # 根因：本块只说"供参考复用"，模型把历史需求文本当成了需求来源。
    # 修复=标题去"相关历史任务"模糊性 + 末尾硬锚（当前对话 > 历史经验，场景冲突时禁止套用）。
    # 修复（fix3_probe R2b 实锤）：硬锚必须拼在正文截断之后——此前锚拼完再整体
    # [:MAX_INJECT_CHARS] 截断，经验行长时锚被切尾截掉，防线形同虚设。
    body = "\n".join(lines)[:MAX_INJECT_CHARS]
    block = (
        "【历史会话经验沉淀（注意：这些是往期会话的记录，**不是本次需求**）】\n"
        + body
        + "\n\n【硬性规则】以上均为历史经验，仅供方法论参考。本次需求**一律以本轮对话中"
        "用户所说为准**：历史需求场景与本次对话的需求不一致时，**严禁**把历史场景"
        "（如园区/门禁/其他客户的方案）当作本次客户场景来产出交付物；如需借鉴，只能借"
        "做法与结构，不能借场景与数字。"
    )
