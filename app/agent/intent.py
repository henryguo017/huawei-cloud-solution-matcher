...
_INDUSTRY_KEYWORDS = [
    ...
    "出海", "工业", "政务云",
    # P1-B 覆盖度补充：6 个新行业包里已声明、但原表缺失的高频二级别名。
    # 未进此表时，用户只说"电网/地产/高校"而不带主词（能源/园区/教育）→ industries 为空 → 包挂不上。
    "电力", "电网", "电厂", "风电", "储能", "地产", "房地产", "高校", "学校", "旅游",
    "智慧交通", "港口", "养殖", "种植",
]
...
# 业务闭环 E2E 修复（2026-09-13）：补"整理成/转成/转换成/做成/变成"变体——
# 真实场景实测"把方案整理成PPT"漏判成 general → FC 模型编造"不能生成 PPT"。
_EXPORT_RE = re.compile(
    r"(导出|下载|另存)[^。]{0,12}(word|pdf|pptx?|文档|方案书|报告)|(导出成|导出为)"
    r"|(生成|写|做)[^。]{0,3}(word|pdf|pptx?|文档)(?:文件|文档)?"
    r"|(整理成|转成|转换成|做成|变成|输出成|输出为)[^。]{0,4}(word|pdf|pptx?|文档|方案书|报告)",
    re.I,
)
...
_PPT_GEN_RE = re.compile(
    r"(生成|做一个?|做份?|做个|输出|来一份?|帮我做|帮我生成|整理成|转成|转换成|做成|变成)[^。]{0,8}pptx?"
    r"|(生成|做|输出|来一份?|整理成|转成|转换成|做成|变成)[^。]{0,8}(幻灯片|演示文稿)",
    re.I,
)
