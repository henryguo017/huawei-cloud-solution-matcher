...
# 全部默认关闭/保守值，保证不开启时行为与现状 100% 一致。
# 开启示例：在 .env 设置 ENABLE_HYBRID_RETRIEVAL=true 等后重启服务。
...
# SSE 流式连接治理
SSE_HEARTBEAT_ENABLED = os.getenv("SSE_HEARTBEAT_ENABLED", "false").lower() == "true"  # 开启后发心跳+超时清理
