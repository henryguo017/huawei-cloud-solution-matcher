// [版本] v3.0.0 — 华为红 12 页 PPT 导出引擎；Agent 对话工作台（Plan 点亮/真实算价/行业技能包）；钉钉飞书群通知；客户管理 CRM；快速体验 Demo
...
// 历史/只读成本卡片渲染（提到顶层，HistoryUI._openDetailModal 可直接调用）
window.renderCostReferenceReadOnly = function renderCostReferenceReadOnly(item) {
    var card = document.getElementById('detail-cost-reference-card');
    if (!card) return;
    var industry = (item && item.industry) || '';
    try {
        var docs = (item && item.source_documents) || (item && item.sources) || [];
        var src = docs[0];
        if (!industry && src && src.metadata) industry = src.metadata.industry || '';
    } catch (e) {}
    API.getPricingReference(industry).then(function(data) {
        if (!data || !data.items || data.items.length === 0) {
            card.style.display = 'none';
            return;
        }
        card.style.display = 'block';
        var badge = document.getElementById('detail-cr-industry-badge');
        if (badge) badge.textContent = data.industry + (data.is_default ? '（通用）' : '');
        var sub = document.getElementById('detail-cr-sub');
        if (sub) sub.textContent = data.description || '基于公开价目表策展的区间参考';
        var disc = document.getElementById('detail-cr-disclaimer');
        ...
        var rows = data.items.map(function(it) {
            ...
            var mid = (it.tier && it.tier.mid) || { qty: it.qty || 1, unit_price: it.ref_price || 0 };
        ...
        var table = document.getElementById('detail-cr-table');
        if (table) {
            var html = '<div class="cr-row cr-row-head"><span class="cr-c-product">产品 / 规格</span><span class="cr-c-qty">数量</span><span class="cr-c-price">单价(月)</span><span class="cr-c-sub">小计</span></div>';
            var total = 0;
            rows.forEach(function(r) {
                if (r.kind === 'biz' || r.kind === 'noprice' || r.kind === 'free') {
                    var label = r.kind === 'biz' ? '商务定价' : (r.kind === 'free' ? '基础免费' : '参考价待补充');
                    var cls = r.kind === 'biz' ? 'cr-row-biz' : (r.kind === 'free' ? 'cr-row-free' : 'cr-row-noprice');
                    html += '<div class="cr-row ' + cls + '"><span class="cr-c-product"><b>' + window._crEsc(r.product) + '</b><br><small>' + window._crEsc(r.spec) + '</small></span><span class="cr-c-biz-note">' + label + '：' + window._crEsc(r.note) + '</span></div>';
                    return;
                }
                var subAmt = (Number(r.qty) || 0) * (Number(r.unit_price) || 0);
                total += subAmt;
                var bill = window._crEsc(r.billing) + (r.unit_label ? ((r.billing ? ' · ' : '') + window._crEsc(r.unit_label)) : '');
                var warn = (!r.verified) ? ' <span class="cr-warn" title="待官网复核">⚠</span>' : '';
                html += '<div class="cr-row"><span class="cr-c-product"><b>' + window._crEsc(r.product) + '</b><br><small>' + window._crEsc(r.spec) + '</small>' + (bill ? '<span class="cr-bill">' + bill + '</span>' : '') + warn + '</span><span class="cr-c-qty">' + r.qty + '</span><span class="cr-c-price">¥' + window._crMoney(r.unit_price) + '</span><span class="cr-c-sub">¥' + window._crMoney(subAmt) + '</span></div>';
            });
            table.innerHTML = html;
            var el = document.getElementById('detail-cr-total');
            if (el) el.textContent = '¥' + window._crMoney(Math.round(total * 100) / 100);
            var lbl = document.getElementById('detail-cr-total-label');
...
const SettingsManager = {
    ...
    async updateSystemInfo() {
        try {
            ...
            if (versionEl) versionEl.textContent = data.version || 'v3.0.0';
            if (updateEl) updateEl.textContent = `API版本: ${data.version || 'v3.0.0'}`;
...
/* ==================== 视图管理：Agent / 经典双视图（纯切换器） ====================
 * 隔离铁律：本模块是"开关"，不是"控制器"。
 *   - 只负责改 body.view-agent / body.view-classic 类 + 同步顶栏胶囊 active 态
 *   - 绝不触碰任一分支的内部状态/页面/全局变量（State / PageTransition 等）
 *   - 经典页面状态由经典模块自己维护；Agent 由 agent_workspace.js 维护
 *   - 两分支的显隐完全由 CSS（body.view-* 选择器）决定，本模块不操作任何业务 DOM
 */
const ViewManager = {
    VIEW_KEY: 'huawei_view_preference',
    _current: undefined, // 运行时当前视图，用于生命周期判断

    // 移动端判定（2026-09-24 解锁后保留）：UA 命中手机关键词 或 视口 ≤820px 即判为移动端。
    // 历史：2026-09-02 范式曾用本判定把移动端锁死在经典模式；2026-09-24 起手机端开放
    // Agent 视图（Agent 工作台自带 ≤820px 抽屉式响应式布局），本判定保留为单一判定源，
    // 供后续移动端体验逻辑复用（当前 ViewManager 自身不再据此拦截）。
    isMobile() {
    ...
    getView() {
        // 2026-09-24 解锁：移动端不再强制经典，尊重历史偏好（无偏好仍默认经典）。
        // 手机端进入 Agent 视图由 agent_workspace.css 的 ≤820px 断点负责布局适配。
    ...
    async setView(v) {
        ...
        if (window.TaskGuard) {
            const allow = await window.TaskGuard.confirmSwitch();
            if (!allow) return; // 用户选择留在本页
        }
        // 切换瞬间禁用 .app-main 的 margin-left 过渡，避免内容因 220↔0 动画产生水平滑动（拼凑感）。
        // 两视图内容最终都落在 x=220，关掉过渡即瞬时对齐、无位移；下一帧恢复过渡（侧栏收起等正常动画不受影响）。
    ...
    // 无条件应用视图状态：设置 localStorage + body 类 + 同步胶囊 active + 触发视图生命周期。
    // 用于初始化（硬刷后必须让 body 类与 localStorage 对齐，否则会露出另一分支）。
    _apply(v) {
        const prev = this._current;
        this._current = v;
        localStorage.setItem(this.VIEW_KEY, v);
        document.body.classList.remove('view-agent', 'view-classic');
        document.body.classList.add('view-' + v);
        this._syncViewToggle();
        // 视图生命周期：Agent 模块自己负责渲染/清理
        if (prev === 'agent' && v !== 'agent' && window.AgentWorkspace && typeof window.AgentWorkspace.beforeLeave === 'function') {
            window.AgentWorkspace.beforeLeave();
        }
        if (v === 'agent' && window.AgentWorkspace && typeof window.AgentWorkspace.init === 'function') {
            window.AgentWorkspace.init();
        }
    },

    // 顶栏切换胶囊：两个选项都可见，active 红高亮表达当前模式
    _syncViewToggle() {
        const v = this._current || this.getView();
        document.querySelectorAll('.view-option').forEach(b =>
            b.classList.toggle('active', b.dataset.view === v));
    },

    init() {
        // 2026-09-24 解锁：移动端不再加 mobile-lock-classic / 强制经典视图，
        // 统一按历史偏好应用视图（无偏好默认 classic）。手机端 Agent 布局由
        // agent_workspace.css ≤820px 断点（侧栏抽屉化）承接。
        // 硬刷后必须无条件应用：即便目标视图 == 已存值，也要确保 body 类已设置
        // （setView 会在 v===_current 时早退，故这里直接走 _apply 绕过守卫）
        this._apply(this.getView());

        // 顶栏切换胶囊：点击对应选项 → 切换视图（移动端胶囊紧凑化显示，点击可切换）
...
const DashboardUI = {
    ...
    renderInfoBar(stats) {
        ...
        if (versionEl) versionEl.textContent = stats.version || 'v3.0.0';
...
const HistoryUI = {
    ...
    _openDetailModal(item, type) {
        ...
        if (type === 'match') {
            try { window.renderCostReferenceReadOnly(item); } catch (e) { console.warn('历史成本卡片渲染失败', e); }
        }
...
function initEventListeners() {
    ...
    matchBtn?.addEventListener('click', async () => {
        ...
        if (window.TaskGuard) window.TaskGuard.begin('classic-match', '方案生成中', function () { if (State.abortControllers.match) State.abortControllers.match.abort(); });
        ...
        try {
            ...
            matchBtn.classList.remove('btn-cancel');
            State.abortControllers.match = null;
            if (window.TaskGuard) window.TaskGuard.end('classic-match');
        }
    });
    ...
    const SolutionChapterRefine = {
        ...
        bind() {
            const cached = State.resultCache && State.resultCache.solution;
            if (!cached || !cached.answer) return;
            // 支持方案匹配页 (#solution-content) + 历史详情页 (#detail-solution-content)
            const containers = [
                document.getElementById('solution-content'),
                document.getElementById('detail-solution-content')
            ].filter(Boolean);
            containers.forEach(contentEl => {
                ...
                contentEl.querySelectorAll('h3').forEach(h3 => {
                    ...
                    btn.addEventListener('click', function(e) {
                        e.stopPropagation();
                        SolutionChapterRefine._openPanel(title, h3, contentEl);
                    });
                    h3.appendChild(btn);
                });
            });
        ...
        // 弹出精修输入面板（container 是所属结果容器：方案页 #solution-content 或历史详情 #detail-solution-content）
        _openPanel(chapterTitle, h3, container) {
            ...
            panel.querySelector('.chapter-refine-ok').addEventListener('click', () => {
                ...
                self._refineChapter(chapterTitle, currentContent, text, panel, h3, container);
            });
        ...
        async _refineChapter(chapterTitle, currentContent, followUp, panel, h3, container) {
            ...
            try {
                ...
                const contentEl = container || document.getElementById('solution-content');
    ...
    analyzeBtn?.addEventListener('click', async () => {
        ...
        if (State.loadingStates.analyze) {
            ...
            if (window.TaskGuard) window.TaskGuard.end('classic-analyze');
        ...
        if (window.TaskGuard) window.TaskGuard.begin('classic-analyze', '竞争分析中', function () { if (State.abortControllers.analyze) State.abortControllers.analyze.abort(); });
...
const FollowUpUI = {
    ...
    async sendFollowUp() {
        ...
        try {
            // 语法糖：输入 "@第N章 ..." 自动路由到章节精修（避免和整篇追问混用）
            const chapterMatch = text.match(/^@第(\d+)\s*章\s*(.*)$/);
            if (chapterMatch) {
                const idx = parseInt(chapterMatch[1], 10) - 1;
                const userText = chapterMatch[2].trim() || '精修该章节';
                const cached = State.resultCache && State.resultCache.solution;
                if (cached && cached.answer) {
                    // 解析 ## 章节，按索引匹配
                    const sections = (cached.answer.match(/^##\s+(.+)$/gm) || []).map(s => s.replace(/^##\s+/, '').trim());
                    if (idx >= 0 && idx < sections.length) {
                        const chapterTitle = sections[idx];
                        const chapterContent = SolutionChapterRefine._extractChapterContent(chapterTitle, cached.answer);
                        const data = await API.refineChapter(
                            cached.demand || '', cached.answer, chapterTitle, chapterContent, userText
                        );
                        const refined = (data.refined_content || '').trim();
                        if (refined) {
                            cached.answer = replaceChapterInMarkdown(cached.answer, chapterTitle, refined);
                            if (cached.solution_json && Array.isArray(cached.solution_json)) {
                                cached.solution_json.forEach(ch => {
                                    if (ch.title && ch.title.trim() === chapterTitle.trim()) ch.content = refined;
                                });
                            }
                            // 重新渲染整篇（章节按钮重新挂载）
                            const container = document.getElementById('solution-content');
                            if (container) container.innerHTML = UI.renderMarkdown(cached.answer);
                            SolutionChapterRefine.bind();
                            this.currentSolution = cached.answer;  // 同步追问侧缓存
                            this.history.push({ role: 'ai', content: `已精修第${idx + 1}章「${chapterTitle}」：` + refined });
                            this.renderHistory();
                            UI.showToast(`已精修第 ${idx + 1} 章`, 'success');
                            return;
                        }
                    }
                }
                UI.showToast(`未找到第 ${idx + 1} 章，请检查章节编号`, 'warning');
                return;
            }
            // 否则按原追问：整篇优化
...
const UpdateTicker = {
    items: [
        '<b>华为红 12 页 PPT 导出</b>：方案一键生成满版商务模板，成本数字自动写入，说"给我生成一个PPT"也行',
        '<b>Agent 对话工作台</b>：执行计划实时点亮、成本计算器真实算价、五大行业专家口径，出稿前质量自检',
        '<b>钉钉 / 飞书群通知</b>：方案完成自动推送免登录分享卡片，群里点开就能看，30 天有效',
        '<b>客户管理 CRM</b>：客户档案 + 方案时间线一键归集，匹配自动参考客户行业精准检索',
        '<b>快速体验</b>：顶栏「体验」一键跑三个行业 Demo，匿名可用，1 分钟看懂平台'
    ],
...
/* ==================== 科技资讯弹窗（RSS 每日聚合） ==================== */
const NewsModal = {
    _cache: {},          // tab -> {items, updated_at, stale}（内存缓存，刷新后预加载）
    _prefetched: false,  // 预加载标记（避免重复预取）
    ...
    init() {
        ...
        // 优化①预加载：页面加载完成后后台静默预取三个 tab，打开即用（无需"加载中"）
        setTimeout(function() { self.prefetch(); }, 400);
    },
    ...
    prefetch() {
        ...
        const self = this;
        Object.keys(this._tabsConfig).forEach(function(tab) {
            const cfg = self._tabsConfig[tab];
            fetch(Config.API_BASE_URL + cfg.endpoint)
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data && data.ok) {
                        self._cache[tab] = { items: data.items || [], updated_at: data.updated_at || '', stale: !!data.stale };
                    }
                })
                .catch(function() { /* 预加载失败静默，打开时再拉取 */ });
        });
    },
    ...
    _switchTab(tabName) {
        ...
        // 优化③切换动效：有缓存 → 立即渲染 + 淡入；无缓存 → 异步 fetch
        if (this._cache[tabName]) {
            this._renderFromCache(tabName);
        } else {
            this._fetch(false);
        }
    },
    ...
    open() {
        ...
        if (this._cache[this._currentTab]) {
    ...
    _renderFromCache(tab) {
        const c = this._cache[tab];
        if (!c) return;
        const self = this;
        // 淡出 → 替换内容 → 淡入
        this._fadeOut(function() {
            self._render(c.items, tab);
            self._updateUpdated(c.updated_at, c.stale);
            self._fadeIn();
        });
    },

    // ---- 优化②：串页防护（请求带 tab 标记，过期响应丢弃；渲染按请求 tab） ----
    _fetch(force) {
        const self = this;
        const reqTab = this._currentTab;
        const cfg = this._tabsConfig[reqTab] || this._tabsConfig.industry;
        // 未加载过 → 显示加载占位；已加载过 → 保留旧内容淡出（避免闪烁）
        if (!this._cache[reqTab]) {
            this.listEl.innerHTML = '<div class="news-loading">加载中…</div>';
        }
        this.listEl.classList.remove('news-visible');
        fetch(Config.API_BASE_URL + cfg.endpoint + (force ? '?force=1' : ''))
            .then(function(r) { return r.json(); })
            .then(function(data) {
                // 串页防护：请求发出时的 tab 已不是当前 tab → 丢弃过期响应
                if (reqTab !== self._currentTab) return;
                ...
                self._cache[reqTab] = { items: data.items, updated_at: data.updated_at || '', stale: !!data.stale };
                self._render(data.items, reqTab);
                self._updateUpdated(data.updated_at, data.stale);
                // 淡入
                requestAnimationFrame(function() { self.listEl.classList.add('news-visible'); });
            })
            .catch(function() {
                if (reqTab !== self._currentTab) return;
    ...
    _updateUpdated(updatedAt, stale) {
        if (this.updatedEl) {
            this.updatedEl.textContent = (updatedAt ? '更新于 ' + updatedAt + (stale ? '（缓存）' : '') : '');
        }
    },

    _render(items, tabName) {
        const self = this;
        if (!items.length) {
...
async function init() {
    try {
        console.log('[Init] 华为云方案匹配系统 v3.0.0 正在初始化...');
        const canvas = document.getElementById('particle-canvas');
        if (canvas) {
        ...
        try { ViewManager.init(); } catch (e) { console.warn('[Init] ViewManager 初始化失败:', e.message); }
