   ...
   旧版 compose-input 在 v=20260824f 移除，但保留 def 逻辑不报错，详见 _renderWelcome。 */
            ...
            .then(function (r) {
                ...
                function handle(ev) {
                    ...
                    if (t === 'agent_phase') {
                        ...
                        // P2-Skills / P1-B：技能包挂载提示（单步重跑路径同样可见）；kind=capability 为能力包
                        self._appendThinkingStep((ev.kind === 'capability' ? '已挂载能力包：' : '已挂载行业技能包：') + (ev.industry || '') + (ev.version ? '（v' + ev.version + '）' : ''), 'phase');
        ...
        _run: function (message) {
            ...
            function onEvent(ev) {
                ...
                if (t === 'plan') {
                    ...
                    // P2-Skills / P1-B：技能包挂载提示（思考流可见；不污染方案正文）；kind=capability 为能力包
                    self._appendThinkingStep((ev.kind === 'capability' ? '已挂载能力包：' : '已挂载行业技能包：') + (ev.industry || '') + (ev.version ? '（v' + ev.version + '）' : ''), 'phase');
                } else if (t === 'thought') {
        ...
        _openPermissionSettings: function () {
            ...
            var tools = [
                ...
                ['run_python', '沙箱代码执行', '受限 Python 计算（无网络无写入，≤5 秒）']
            ];
            var def = { web_search: 'allow', web_extract: 'allow', generate_doc: 'ask', read_customer_file: 'ask', run_python: 'ask' };
        ...
        /* 登录态哨兵：1s 轮询（单次 localStorage 读，开销可忽略）。
           只在 token 「有→无」跳变时动作（登出 / token 过期 / 被顶号）：
           清本地会话缓存 + 中断在途流式 + 复位到欢迎态（侧栏立即变空，不留残影）。
           不读写经典全局对象、不 query 经典元素，符合分支隔离铁律。 */
        _startAuthWatch: function () {
            if (this._authWatchTimer) return;
            var self = this;
            this._authWatchTimer = setInterval(function () {
                var has = !!self.userToken();
